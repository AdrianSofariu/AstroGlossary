...
export function DropdownMenuCheckboxes() {
  ...
  const [tempCheckedTypes, setTempCheckedTypes] = React.useState<Record<string, boolean>>({});
  ...
  return (
    <DropdownMenu onOpenChange={(open) => !open && handleDropdownClose()}>
      <DropdownMenuTrigger asChild>
        <Button type="submit" variant="outline" size="icon" className="hover:bg-pink-600 transition">
        <Filter/>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="w-56">
        ...
        <DropdownMenuSeparator />
        {types.map((type) => (
          <DropdownMenuCheckboxItem key={type} checked={tempCheckedTypes[type] ?? true} 
